...
pub use super::*;
use crate::drivers::message::CanMessage;
use core::{borrow::Borrow, str};
use log::{info, debug, warn, Log};
...
impl AcceptanceFilterMask {
    ...
    pub fn new(mask: u16, filter: u16) -> Self {
        //TODO: - Parse the RXM mode to define the correct filter (Standard vs Extended
        // ID).
    ...
    Mcp2515Driver<SPI, PIN, PININT>
{
    ...
    pub fn init(spi: SPI, cs: PIN, interrupt_pin: PININT, can_settings: Mcp2515Settings) -> Self {
        ...
        if (driver.can_settings.canctrl.mode != OperationTypes::Normal) {
            panic!("Operational Mode, has to be in Normal Mode, before using CAN Bus!");
        }
    ...
    fn transfer_test(&mut self, register: MCP2515Register) {
        ...
        self.cs.set_high();
        //log::info!("Sent: {:08b}, Received: {:08b}", write_buf, read_buf);
    }
    ...
    fn reset_instruction(&mut self) {
        log::trace!("Calling Reset instruction over SPI");
        self.cs.set_low();
    ...
    pub fn interrupt_decode(&mut self) -> Result<InterruptFlagCode, Mcp2515Error> {
        ...
        match InterruptFlagCode::try_from(interrupt_code) {
            Ok(flag_code) => {
                log::info!("Received the Interrupt Code: {:?}", flag_code);
                Ok(flag_code)
            }
            Err(e) => {
                log::error!("Failed to decode InterruptFlagCode: {:?}", e);
    ...
    fn error_handling(&mut self) {
        ...
        log::info!("TEC status: {:08b}", tec);
        log::info!("REC status: {:08b}", rec);
    ...
    fn eflg_decode(eflg_bits: u8, eflags: &mut [EFLG]) -> usize {
        ...
        for bit_pos in 0..8 {
            if (eflg_bits & (1 << bit_pos) != 0) {
                log::info!("Found EFLG: {:?}", EFLG::from_u8(bit_pos as u8));
                eflags[bit_pos] = EFLG::from_u8(bit_pos as u8);
                count += 1;
            }
        }
        log::info!("eflg_decode count: {:?}", count);
        count
    }

    fn message_error_check(&mut self, canintf: u8) {
        if (canintf & (1 << 7) != 0) {
            log::info!("Message Error Interrupt occurred (MERRE)!");
    ...
    pub fn handle_interrupt(
        ...
        match received_interrupt {
            ...
            InterruptFlagCode::ErrorInterrupt => {
                log::info!("Received Error Interrupt, with flags...");
                self.error_handling();
            ...
            InterruptFlagCode::RXB0Interrupt => {
                log::info!("RXB0 received a message!");
            ...
            InterruptFlagCode::RXB1Interrupt => {
                log::info!("RXB1 received a message!");
